...
type Comment interface {
	...
	GetCommentByCommentID(commentID int) (*module.Comment, error)
}
...
func (r *CommentRepository) GetCommentByCommentID(commentID int) (*module.Comment, error) {
	c := &module.Comment{}
	err := r.db.QueryRow("SELECT id FROM comments WHERE id = ?", commentID).Scan(&c.ID)
	if err == sql.ErrNoRows {
		return nil, err
	}
	return c, nil
